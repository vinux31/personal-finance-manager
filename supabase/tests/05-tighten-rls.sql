...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'FAIL: aggregate_by_period(NULL) unexpectedly failed for non-admin';
  END IF;
  ...
  ELSE
    RAISE NOTICE 'FAIL: aggregate_by_period(p_user_id = own) unexpectedly failed for non-admin';
  END IF;
  ...
  ELSE
    RAISE NOTICE 'FAIL: admin SELECT profiles returned only % rows (View-As broken)', v_count;
  END IF;
  ...
  ELSE
    RAISE NOTICE 'FAIL: admin SELECT allowed_emails returned 0 rows (unexpected)';
  END IF;
...
\echo 'Expected: 14 PASS notices (Section 1: 5, Section 2: 2, Section 3: 4, Section 4: 3).'
\echo '============================================================'
