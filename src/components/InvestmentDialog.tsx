...
export default function InvestmentDialog({
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setCurrentPriceStr(
        editing.current_price != null ? String(editing.current_price) : '',
      )
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const finalType =
      assetTypeSel === CUSTOM ? customType.trim() : assetTypeSel
    const qty = Number(qtyStr)
    const buyPrice = parseRupiah(buyPriceStr)
    const currentPrice =
      currentPriceStr.trim() === '' ? null : parseRupiah(currentPriceStr)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>
              {editing ? 'Edit Investasi' : 'Tambah Investasi'}
            </DialogTitle>
            <DialogDescription>
              Catat aset yang Anda miliki.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                id="inv-name"
                placeholder="Contoh: BBCA, Reksadana ABC"
                value={assetName}
            ...
            <div className="grid gap-2">
              ...
              {buyPriceStr && (
                <p className="text-xs text-muted-foreground">
                  Total modal:{' '}
                  {formatRupiah(
                    (Number(qtyStr) || 0) * parseRupiah(buyPriceStr),
                  )}
                </p>
              )}
            </div>
