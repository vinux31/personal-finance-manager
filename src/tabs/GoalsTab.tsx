...
import { useGoals, useDeleteGoal, goalProgress, type Goal } from '@/queries/goals'
              ...
              <div key={g.id} className="rounded-lg border bg-card p-5">
                ...
                <div className="mt-4 space-y-1.5">
                  <div className="flex items-baseline justify-between text-sm">
                    <span className="font-medium">{formatRupiah(g.current_amount)}</span>
                ...
                <div className="mt-4 flex justify-end gap-1">
                  <Button variant="outline" size="sm" onClick={() => { setAddMoneyFor(g); setAddMoneyOpen(true) }} disabled={g.status === 'completed'}>
                  ...
                  <Button variant="ghost" size="icon" onClick={() => onDelete(g)}><Trash2 className="h-4 w-4 text-red-600" /></Button>
                </div>
              </div>
            )
          })}
