...
must_haves:
  truths:
    ...
    - "Minimum 4 [[term]]X[[/term]] markers tersebar di body strings modulContent.ts"
  artifacts:
    - path: "package.json"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - At least 4 `[[term]]X[[/term]]` markers inserted in body strings wrapping glossary terms (kemunculan pertama per H2 section)
...
<p>Bias kognitif yang paling sering memakan portofolio:</p>`,
  ...
  <acceptance_criteria>
    ...
    - `grep -cE '\[\[[a-z-]+\]\]' src/data/modulContent.ts` outputs ≥`8` (open markers count, expect ~9 valid+sentinel; corresponds to 4+ unique glossary term invocations)
